...
# Google API client libs are lazy-imported later to avoid hard failure if 'packaging' isn't present yet.
...
def parse_topics_from_body(body):
    ...
    def collect_after_marker(marker):
        ...
        return out[:3]
    t = collect_after_marker("new topic options:") or collect_after_marker("choose one:")
    if t: return t
    try:
...
def llm_script(trending_query, word_hint="100–130"):
    if not GROQ_API_KEY:
...
- Style: energetic, plain language, second-person; target {word_hint} words so the final video lands ~50–58s.

Output pure JSON with:
- voiceover: string (full narration)
...
def chunk_voiceover(text, max_words=7):
...
def make_srt_from_voiceover(voiceover_text, duration, path):
    chunks = chunk_voiceover(voiceover_text, max_words=7)
    if not chunks:
        chunks = ["Keep it simple", "Move, hydrate, rest", "Small steps add up"]
    n = len(chunks)
    step = max(0.9, duration / n)
    t = 0.4
    out = []
...
def burn_subs(in_mp4, srt_path, out_mp4):
    # Small, bright, stroked, bottom-aligned captions
    style = (
...
# ---------- Rendering (no cropping; letterbox/pad to 1080x1920) ----------
def letterbox_clip(clip, target_w=1080, target_h=1920, bg_color=(0,0,0)):
    ...
    return comp.set_duration(c2.duration).set_audio(None)
...
def render_and_cap(broll_urls, voice_mp3, temp_mp4, final_mp4, voiceover_text, target_h=1920, target_w=1080):
    ...
    for p in local:
        ...
        c = c.subclip(0, take)
        clips.append(letterbox_clip(c, target_w=target_w, target_h=target_h))

    merged = concatenate_videoclips(clips, method="compose")

    # Narration only for temp render
    ...
    # Build captions from exact voiceover text and burn them bottom-centered
    srt_path = "cap.srt"
...
# ---------- YouTube (lazy import with packaging fallback) ----------
...
# ---------- Preview upload with fallback ----------
def compress_for_preview(in_path, out_path="preview_small.mp4", target_w=720, target_h=1280):
    # Scale-to-fit within 720x1280 and pad (no crop); lower bitrate for smaller upload
...
def upload_preview_fallback(video_path):
    # Try smaller preview for better success on free hosts
    try:
    ...
    try:
        with open(path_small, "rb") as f:
            r = requests.post("https://0x0.st", files={"file": (os.path.basename(path_small), f, "video/mp4")}, timeout=120)
...
def upload_preview_youtube_or_fallback(video_path, title, description, tags):
    try:
        ...
        raise RuntimeError(f"Preview upload failed and fallback unavailable: {msg}")

# ---------- Metadata helpers ----------
def upload_preview(video_path, title, description, tags):
...
def post_preview_comment(owner, repo, number, meta, slot):
    ...
    if meta.get("yt_upload_blocked"):
        note = "\nNote: YouTube upload limit reached. This preview is hosted temporarily; use /regenerate-video after the limit resets to upload."
    msg = (
        f"Preview ready (attempt {meta['attempt']}, {meta['duration_sec']}s): {meta['preview_link']}{note}\n"
...
#Shorts #health #wellness"""
            info = upload_preview(final, s["title"], desc, s.get("tags",""))
            meta = {
                "topic": topic,
                "title": s["title"],
                "description": desc,
...
def post_preview_comment(owner, repo, number, meta, slot):
    note = ""
    if meta.get("yt_upload_blocked"):
        note = "\nNote: YouTube daily upload limit was reached. The preview is hosted temporarily. Use /regenerate-video after the limit resets to upload."
    msg = (
        f"Preview ready (attempt {meta['attempt']}, {meta['duration_sec']}s): {meta['preview_link']}{note}\n"
        f"Reply:\n- /approve-video (schedule PRIVATE → auto-publish next day {slot})\n"
        f"- /reject-video (delete preview and pick a new topic)\n\n"
...
Educational only, not medical advice. Consult a qualified professional for personal guidance.
#Shorts #health #wellness"""
            info = upload_preview_youtube_or_fallback(final, s["title"], desc, s.get("tags",""))
            meta = {
                "topic": topic,
                "title": s["title"],
                "description": desc,
...
def safe_main():
    ...
    if comment.lower().startswith("/custom-topic"):
        ...
        if not topic:
            ...
            return
        meta, new_body = build_preview_until_under_58(topic, slot, body, max_attempts=4)
        if not meta:
    ...
    if comment.lower().startswith("/regenerate-video"):
        ...
        if not topic:
            ...
            return
        meta2, new_body = build_preview_until_under_58(topic, slot, body, max_attempts=4)
        if not meta2:
            post_comment(owner, repo, number, "Couldn't get under 58s after attempts. Use /new-topic or /custom-topic.")
            return
        gh("PATCH", f"https://api.github.com/repos/{owner}/{repo}/issues/{number}", json={"body": new_body})
    ...
    if comment.lower().startswith("/approve-video"):
        meta = get_metadata_from_issue_body(body) or {}
        if meta.get("yt_upload_blocked"):
            post_comment(owner, repo, number, "YouTube upload limit was reached for this preview. Please /regenerate-video after the limit resets (typically within 24 hours).")
            return
